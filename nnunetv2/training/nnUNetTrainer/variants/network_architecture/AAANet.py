    ...
    def __init__(
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                ...
                nn.init.constant_(m.bias, 0)
        if mamba:
            self.mamba = MambaLayer(dim=out_channels, d_state=16, d_conv=4, expand=2)
        else:
            self.mamba = None
...
class MultiQueryAttention(nn.Module):
    ...
    def forward(self, tensor1, tensor2):
        ...
        K = K.view(B, -1, self.head_dim).unsqueeze(1).expand(-1, self.num_heads, -1, -1)
        V = V.view(B, -1, self.head_dim).unsqueeze(1).expand(-1, self.num_heads, -1, -1)
...
class AAANet(nn.Module):
    def __init__(
        ...
        self.aneurysm.append(nn.Conv3d(in_channels, encoder_channels[0], 3, 1, 1))
        ...
        ckpt = torch.load(
            "/data1/songwei/WorkStation/Data/results/Dataset111_CAS/baseline/fold_2/checkpoint_best.pth"
        )["network_weights"]
        for ckpt_key in list(ckpt.keys()):
            if "encoders" in ckpt_key:
                ckpt[ckpt_key.replace("encoders.", "")] = ckpt.pop(ckpt_key)
            else:
                ckpt.pop(ckpt_key)
        self.encoders.load_state_dict(ckpt)
        # self.encoders 的权重冻结
